...
export function refreshTable() {
  ...
  const isArchivedBtnClicked = refs.archiveBtn.classList.contains('is-clicked');

  refs.notesTableBody.innerHTML = !isArchivedBtnClicked
    ? notes
        .map(note => {
          const { id, name, icon, created, category, content, isArchived } = note;
          const contentCheck = content.length > 20 ? content.slice(0, 20) + '...' : content;
          const datesCheck = note.dates ? note.dates : '';

          if (isArchived === false) {
            return `<tr id="${id}" class="table-notes__row " >
                    ...
                    <td>
                        <div id="${id}" class='table-notes__btn-block'>
    ...
    : notes
        .map(note => {
          const { id, name, created, category, content, isArchived } = note;
          const contentCheck = content.length > 20 ? content.slice(0, 20) + '...' : content;
          const datesCheck = note.dates ? note.dates : '';
          if (isArchived === true) {
